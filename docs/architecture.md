...
`lib/unassisted.dart` 提供原始状态与固定动作含义。JEV 方块每次执行一个 `left/right/rotate/down/hard_drop`，不调用落点规划或辅助提示。非法输入由游戏规则拒绝并暂停，不生成替代动作。“AI 直接落位”禁用，模型仍可自行选择 `hard_drop`。

JEV 贪吃蛇独玩不构造 `SnakeCycle`，保留引擎自然出生位置；竞技按玩家分别选择请求格式，JEV 槽位跳过 `ArenaAssist` 纠错，即使另一方开启辅助。下述辅助数据流适用于其他模型。规则校验与碰撞结算仍由游戏引擎执行。
